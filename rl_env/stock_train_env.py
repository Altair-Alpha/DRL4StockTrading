...
class StockTrainEnvV1(gym.Env):
    ...
    def __init__(self, daily_data: Dict[int, pd.DataFrame], stock_codes: List[str], verbose: bool=True):
        ...
        # 此问题中无法对状态空间进行标准化
        self.observation_space = spaces.Box(low=-np.inf, high=np.inf, shape=(1 + 6*STOCK_DIM,))
    ...
    def step(self, action: np.ndarray):
        if not self.done:
            action = action * SHARES_PER_TRADE

            # 总资产为：余额 + 每只股票持股 * 当前日期股价
            begin_total_asset = self.state[0] + sum(
                [shares*price for shares, price in zip(self.state[1: STOCK_DIM+1],
                                                       self.state[STOCK_DIM+1: 2*STOCK_DIM+1])])
            self.asset_memory.append(self.state[0 : STOCK_DIM+1] + [begin_total_asset]) # state中 0至STOCK_DIM 项为（余额，持股）
            ...
            # 取新一日的股票数据，更新state
            self.state = self.state[0 : 1+STOCK_DIM] + self.data[self.cur_date]['close'].tolist() \
            ...
            if self.verbose:
                ...
                print('StockTrainEnvV1:', 'after trade, total asset:', begin_total_asset, end_total_asset)
                print('StockTrainEnvV1:', 'reward:', self.reward)
        else:
            save_result(self.dates, self.asset_memory, self.reward_memory)
    ...
    def reset(self):
        ...
        self.state = [INITIAL_BALANCE] + [0] * STOCK_DIM + self.data[self.cur_date]['close'].tolist() \
                     + self.data[self.cur_date]['macd'].tolist() \
                     + self.data[self.cur_date]['kdjk'].tolist() \
                     + self.data[self.cur_date]['kdjd'].tolist() \
                     + self.data[self.cur_date]['rsi'].tolist()
    ...
    def _buy_stock(self, index, volume):
        max_volume = self.state[0] // self.state[1 + STOCK_DIM + index] # 账户资金最多能购买的股数
        real_volume = min(volume, max_volume) # 确保入参volume不超过最大能购买数，避免交易后账户资金为负

        # 更新资金，减少：股价*购买股数*(1+交易费率)
        amount = self.state[1 + STOCK_DIM + index] * real_volume  # A股印花税为卖方单向收费，其余小额双向收费项暂且不计
    ...
    def _sell_stock(self, index, volume):
        ...
        if self.state[1 + index] > 0:
            ...
            amount = self.state[1 + STOCK_DIM + index] * real_volume\
                     * (1 - TRANSACTION_FEE_PERCENTAGE) # A股印花税为卖方单向收费，其余小额双向收费项暂且不计
            prev_balance = self.state[0]
            self.state[0] += amount
            # 更新持股
            self.state[1 + index] -= real_volume
...
def save_result(dates, assets, rewards):
    # 绘制资产变化图
