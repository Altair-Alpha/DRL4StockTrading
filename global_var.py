...
EVAL_MAX_PERCENTAGE_PER_TRADE   = 0  # 每步交易最大金额占当前总资金量比例（测试）
TRAIN_START_DATE                = 0  # 训练开始日期
TRAIN_END_DATE                  = 0  # 训练结束日期
EVAL_START_DATE                 = 0  # 测试开始日期
EVAL_END_DATE                   = 0  # 测试结束日期
...
def read_config():
    ...
    global INITIAL_BALANCE, TRANSACTION_FEE_PERCENTAGE, TRAIN_MAX_PERCENTAGE_PER_TRADE, EVAL_MAX_PERCENTAGE_PER_TRADE
    global TRAIN_START_DATE, TRAIN_END_DATE, EVAL_START_DATE, EVAL_END_DATE
    ...
    EVAL_MAX_PERCENTAGE_PER_TRADE  = float(conf.get('stock_env', 'eval_max_percentage_per_trade'))

    TRAIN_START_DATE = int(conf.get('stock_env', 'train_start_date'))
    TRAIN_END_DATE   = int(conf.get('stock_env', 'train_end_date'))
    EVAL_START_DATE  = int(conf.get('stock_env', 'eval_start_date'))
    EVAL_END_DATE    = int(conf.get('stock_env', 'eval_end_date'))
